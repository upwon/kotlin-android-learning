...
export const courseUnits: CourseUnit[] = [
  ...
  },
  {
    ...
    label: "第九部分",
    title: "Jetpack 核心工程",
    description: "把架构建议落进 Hilt、Room、DataStore、WorkManager、Paging 与 Navigation。",
    accent: "amber",
    chapters: [
      chapter(30, "modern-android-architecture", "现代 Android 架构", "UI、Domain、Data、UDF 与状态生产管线。", 78, "进阶", ["UI/Data 分层", "UDF", "状态持有者", "UseCase", "错误建模", "进程恢复"], ["画出可测试的功能边界", "判断逻辑应该放在哪一层"]),
      chapter(31, "hilt-dependency-injection", "Hilt 依赖注入", "组件、作用域、Qualifier、ViewModel 与测试替换。", 86, "进阶", ["依赖注入", "组件层级", "作用域", "Qualifier", "Hilt ViewModel", "测试替换"], ["建立完整 Hilt 依赖图", "避免错误作用域与 Context 泄漏"]),
      chapter(32, "room-advanced", "Room 进阶", "关系、事务、迁移、索引与可验证数据库边界。", 96, "高级", ["Entity 与索引", "关系查询", "事务", "Migration", "Flow 查询", "数据库测试"], ["安全演进数据库 Schema", "为一致性和查询性能设计 DAO"]),
      chapter(33, "datastore", "DataStore", "Preferences、Proto、迁移、异常与数据层集成。", 72, "进阶", ["Preferences DataStore", "Proto DataStore", "Serializer", "迁移", "Flow", "测试"], ["替代 SharedPreferences 保存设置", "设计类型安全且可迁移的配置存储"]),
      chapter(34, "workmanager", "WorkManager", "持久任务、约束、唯一工作、退避与进度。", 88, "高级", ["CoroutineWorker", "约束", "唯一工作", "链式任务", "重试退避", "测试"], ["为必须完成的后台任务选择 WorkManager", "设计幂等且可观察的同步任务"]),
      chapter(35, "paging-3", "Paging 3", "PagingSource、Pager、RemoteMediator、缓存与加载状态。", 105, "高级", ["PagingSource", "Pager", "cachedIn", "LoadState", "RemoteMediator", "Room 集成"], ["构建数据库与网络分页链路", "正确处理刷新、重试和分页错误"]),
      chapter(36, "navigation", "Navigation 与类型安全路由", "路由、返回栈、深链、结果传递与测试。", 92, "进阶", ["类型安全路由", "返回栈", "嵌套图", "Deep Link", "状态恢复", "导航测试"], ["只传最小导航参数", "解释返回栈与深链行为"]),
      chapter(37, "android-testing", "Android 测试体系", "单元、数据库、Worker、导航与 UI 测试分层。", 110, "高级", ["测试金字塔", "协程测试", "Room 测试", "WorkManager 测试", "Espresso", "测试替身"], ["为每层选择合适测试", "写出稳定且不依赖真实时间的测试"]),
    ],
  },
  {
    id: "compose",
    ...
    description: "从声明式心智模型走到状态、导航、自适应、测试与性能。",
    accent: "pink",
    chapters: [
      chapter(38, "compose-mental-model", "Compose 心智模型与重组", "组合、重组、跳过、稳定性与无副作用 UI。", 82, "进阶", ["Composable", "Composition", "Recomposition", "Slot Table", "稳定性", "Preview"], ["解释为什么重组不是整页重绘", "写出幂等且无副作用的 Composable"]),
      chapter(39, "compose-layout-material", "布局、Modifier 与 Material 3", "约束测量、Modifier 顺序、主题与组件设计。", 98, "进阶", ["Row/Column/Box", "Modifier", "约束与测量", "Material 3", "主题", "自定义组件"], ["根据约束构建响应式布局", "理解 Modifier 顺序为何改变结果"]),
      chapter(40, "compose-state", "状态、remember 与状态提升", "Snapshot State、rememberSaveable、派生状态与状态所有权。", 96, "高级", ["mutableStateOf", "remember", "rememberSaveable", "状态提升", "derivedStateOf", "snapshotFlow"], ["选择正确状态生命周期", "把有状态组件拆成可测试无状态组件"]),
      chapter(41, "compose-viewmodel-udf", "Compose、ViewModel 与 UDF", "UiState、事件、生命周期安全收集与页面状态持有。", 88, "高级", ["collectAsStateWithLifecycle", "UiState", "UI Event", "State Holder", "SavedStateHandle", "预览 Fake"], ["连接 StateFlow 与 Compose", "避免 Composable 直接修改业务状态"]),
      chapter(42, "compose-effects", "Effect、协程与生命周期", "LaunchedEffect、DisposableEffect、produceState 与副作用边界。", 92, "高级", ["LaunchedEffect", "rememberCoroutineScope", "DisposableEffect", "produceState", "SideEffect", "rememberUpdatedState"], ["根据副作用寿命选择 Effect", "避免错误 key 导致重复任务"]),
      chapter(43, "compose-lists-forms-paging", "列表、表单与 Paging Compose", "Lazy 布局、稳定 key、表单焦点与分页加载状态。", 112, "高级", ["LazyColumn", "key/contentType", "表单状态", "焦点与键盘", "Paging Compose", "空态与错误态"], ["写出稳定高效的长列表", "完整处理分页刷新与追加状态"]),
      chapter(44, "navigation-compose", "Navigation Compose", "类型安全目的地、嵌套图、深链与返回栈状态。", 96, "高级", ["NavHost", "类型安全 Route", "参数", "嵌套图", "Deep Link", "测试"], ["搭建可测试的 Compose 导航", "避免把 NavController 扩散到整个 UI"]),
      chapter(45, "compose-motion-interoperability", "动画、手势、Insets 与互操作", "动画选择、输入处理、系统栏和 View 渐进迁移。", 104, "高级", ["AnimatedVisibility", "animate*AsState", "手势", "WindowInsets", "AndroidView", "ComposeView"], ["选择合适动画 API", "在 View 与 Compose 间安全迁移"]),
      chapter(46, "compose-adaptive-accessibility", "自适应布局与无障碍", "窗口尺寸、列表详情、多窗、语义与 TalkBack。", 102, "高级", ["Window Size", "列表详情", "折叠屏", "Semantics", "触控目标", "无障碍测试"], ["让同一页面适配手机和平板", "为自定义组件补全语义"]),
      chapter(47, "compose-testing-performance", "Compose 测试、性能与稳定性", "语义树、同步、重组诊断、Macrobenchmark 与 Baseline Profile。", 118, "高级", ["Compose Test Rule", "Semantics", "截图测试", "稳定性", "Macrobenchmark", "Baseline Profile"], ["编写行为导向的 Compose 测试", "用测量结果定位重组与启动问题"]),
    ],
  },
  {
    id: "production-android",
    label: "第十一部分",
    title: "生产级 Android 项目",
    description: "把 Compose 与 Jetpack 合并进离线优先、多模块、可发布的完整应用。",
    accent: "green",
    chapters: [
      chapter(48, "compose-offline-capstone", "Compose 离线优先完整项目", "搜索、分页、详情、收藏、同步、恢复与错误重试。", 240, "高级", ["需求与边界", "Room/Network", "RemoteMediator", "Compose UI", "离线同步", "端到端测试"], ["独立完成现代 Compose 应用主链", "解释离线、一致性与生命周期取舍"]),
      chapter(49, "android-modularization-build", "多模块与构建工程", "模块边界、Version Catalog、Convention Plugin、KSP 与 CI。", 126, "高级", ["模块化", "依赖规则", "Version Catalog", "Convention Plugin", "构建性能", "CI"], ["设计可演进模块图", "让本地与 CI 构建保持一致"]),
      chapter(50, "android-production-quality", "安全、性能与发布质量", "权限、密钥、R8、启动性能、监控与发布回滚。", 150, "高级", ["权限与隐私", "网络安全", "R8", "Baseline Profile", "可观测性", "发布检查"], ["完成生产发布前质量门禁", "用指标验证性能与稳定性"]),
    ],
  },
];
